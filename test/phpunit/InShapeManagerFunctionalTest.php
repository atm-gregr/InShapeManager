...
/**
 * \file    test/phpunit/InShapeManagerFunctionalTest.php
...
/**
 * Class InShapeManagerFunctionalTest
 *
 ...
 *
 * @package Testinshapemanager
 */
class InShapeManagerFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modInShapeManager')->click();
		} else {
			// Disable the module
			$this->byHref('modInShapeManager')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/inshapemanager/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('inshapemanager/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/inshapemanager/admin/about.php');
		$this->authenticate();
		return $this->assertContains('inshapemanager/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('inshapemanagerwidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modInShapeManager_InShapeManagerTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modInShapeManager_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
